...
.. figure:: figs/cookbook/cookbook_vison_config_1.png
    :align: center
    :figclass: align-left
...
.. csv-table:: Correspondence between specific test names and generic test handles.
    :header: "Action", "Comment"
    :widths: 10, 10
...
.. figure:: figs/cookbook/cookbook_vison_config_3.png
    :align: center
    :figclass: align-left
...
.. figure:: figs/cookbook/cookbook_vison_config_2.png
    :align: center
    :figclass: align-left
...
.. csv-table:: Aprox. test processing times
    :header: "Action", "Comment"
    :widths: 5, 3, 3, 10
...
::
    ...
    $ cp /disk/euclid_caldata06/data06/SOFTWARE_LITE/TEMPLATES_CALCAMP/ANALYSIS/vison_config_BLOCK_MMM19.py .
    $ mv vison_config_BLOCK_MMM19.py vison_config_NOSTROMO_APR19.py     
...
.. figure:: figs/cookbook/cookbook_11.png
    :align: center
...
.. figure:: figs/cookbook/cookbook_12.png
    :align: center
...
.. figure:: figs/cookbook/cookbook_13.png
    :align: center
...
.. figure:: figs/cookbook/cookbook_14.png
    :align: center
...
.. figure:: figs/cookbook/cookbook_15.png
    :align: center
...
PRACTICAL CASE: Running the pipeline in "FPA" mode
==================================================
...
We have put together a practical case just to show how the pipeline is run in this mode.

We have put a copy of part of the data from the CEA FPA campaign at:


::

    /disk/euclid_caldata06/data06/TRAIN/TUTORIALS/FPA_CEA
...
#. The **data** is in data/05_12_19/
    ...
    #. the log_*txt are other logs written by the IWS software tool that created the LE1 fits files from the binary files (raw data from the instrument).
    #. the VIS_SPW1N_*_????? files are the **raw binaries**.
    #. the VIS_SPW1N_*_?????_h.json are the json headers generated by IWS software using the metadata in the binary files.
    #. the VIS_SPW1N_*_?????.patch.mosaic.L1.fits are the **LE1 fits** files we'll be using as image inputs.
    #. An EXP_LOG file that we had to create **manually** and that is needed for the pipeline to run. This is where we tell the pipeline which image corresponds to which OBSID, in this type of tests (remember that in the VGCC ELVIS generated an EXP_LOG for us, automatically... now we need to create it ourselves, to be able to keep things more or less the same in the pipeline).
#. **incdps/** contains calibration data-products: just a matrix with gain values for each quadrant in the FPA. These are taken from the VGCC.
#. **references/** contains reference value from the VGCC: offsets, rons, charge injection levels in different conditions.
#. the configuration file template, **vison_config_FPA_XXX21.py**. You can also find this file at: /disk/euclid_caldata06/data06/SOFTWARE_LITE/TEMPLATES/ANALYSIS/.
#. a cheatsheet file with the command to execute the pipeline on one "session" of the configuration file.

Once we have filled in the configuration file correctly, we can execute the pipeline commanding:

::

$ vison_run -F -y vison_config_CEAFPA_DEC19.py -R D21 -t _D21 -l -g -m 6 -C -i
