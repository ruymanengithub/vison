...
class DarkTask(Task):
    ...
    def check_metrics_ST(self,**kwargs):
        ...
        if test == 'DARK01':
            ...
            _compliance_Flu = self.check_stat_perCCD(arr,Flu_lims,CCDs)
            
            if not self.IsComplianceMatrixOK(_compliance_Flu): self.dd.flags.add('POORQUALDATA')
            if self.log is not None: self.addComplianceMatrix2Log(_compliance_offsets,label='COMPLIANCE FLUENCE [%s]:' % reg)        
            if self.report is not None: self.addComplianceMatrix2Report(_compliance_offsets,label='COMPLIANCE FLUENCE [%s]:' % reg)