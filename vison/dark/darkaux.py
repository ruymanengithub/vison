...
def get_DarkDefectsMask(ccdobj,threshold,subbgd=True,extension=-1,bgdmodel=None,
    ...
    if subbgd:
        ...
        if bgdmodel is None:
            ...
            bgd = ccdmod.CCD()
            bgd.add_extension(np.zeros_like(ccdobj.extensions[extension].data,dtype='float32'))
            ...
            for Quad in Quads:
                ...
                Qmodel = ccdobj.get_quad(Quad,canonical=True,extension=extension).copy() # pre/overscans will get nulled
                
                Qmodel[prescan:prescan+Qimgmodel.shape[0],0:Qimgmodel.shape[1]] = Qimgmodel.copy()
...
def get_DarkDefectsMask_CDP(self,darkccdobj,threshold,subbgd=True,extension=-1,bgdmodel=None):
    ...
    meta = OrderedDict(threshold=threshold,subbgd=subbgd,function=get_DarkDefectsMask.__name__)
    
    maskcdp = cdp.CDP(data=data,meta=meta,ID=ID,BLOCKID=BLOCKID,CHAMBER=CHAMBER)
