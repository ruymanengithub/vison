...
class TestCCDClass(unittest.TestCase):
    ...
    def test_get_tile_coos(self):
        ...
        def are_within_image(llpix, urpix, wpx, hpx, img_bounds):
            arewithinimage = [(llpix[i][0] >= img_bounds[0]) & (urpix[i][0] <= img_bounds[1]) &
                              (llpix[i][1] >= img_bounds[2]) & (
                                  urpix[i][1] <= img_bounds[3])
        ...
        for i, Q in enumerate(Qs):
            ...
            self.assertTrue((wpx == itiles['wpx']) and (
                hpx == itiles['hpx']), msg='check 1, %s' % comm_msg)
            self.assertTrue(len(ccpix) == len(llpix) == Nsamps,
                            msg='check 2, %s' % comm_msg)

            urpix = get_urpix(llpix, wpx, hpx)
            arewithinimage = are_within_image(
                llpix, urpix, wpx, hpx, img_bounds)
    ...
    def test_getsectioncollims(self):
        ...
        for iQ, Q in enumerate(self.ccdobj.Quads):
            ...
            self.assertTrue(
                sizesareok, msg='dimensions error... Failed at check %i,Q=%s' % (iQ, Q))
