...
def getacrossrowscut(CCDobj):
    ...
    for iQ,QUAD in enumerate(QUADS):
        ...
        prestart,preend,imgstart,imgend,ovstart,ovend = CCDobj.getsectioncollims(QUAD)
...
def dissectFITS(FITSfile,path=''):
    ...
    for iQ,QUAD in enumerate(QUADS):
        ...
        diss['imgstats'][QUAD] = getsectionstats(CCDobj,QUAD,'image',xbuffer=xbuffer,ybuffer=ybuffer)
        diss['prestats'][QUAD] = getsectionstats(CCDobj,QUAD,'prescan',xbuffer=xbuffer,ybuffer=ybuffer)
        diss['overstats'][QUAD] = getsectionstats(CCDobj,QUAD,'overscan',xbuffer=xbuffer,ybuffer=ybuffer)
