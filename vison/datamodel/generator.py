...
def _update_fromscript(rowdict, scriptcol):
    ...
    scriptkeys = list(scriptcol.keys())
...
def generate_Explog(scrdict, defaults, elvis=context.elvis, explog=None, OBSID0=1000,
    ...
    for iscrcol in range(1, Nscriptcols + 1):
        ...
        N = scriptcol['frames']
        inputkeys = [key for key in list(scriptcol.keys()) if key != 'frames']

        rowdict = {}
        for eckey in expcolkeys:
            rowdict[eckey] = None
        ...
        for subixrow in range(N):
            ...
            for ixCCD in range(1, 4):
                ...
                rowdict['File_name'] = 'EUC_%i_%sD_%sT_ROE1_CCD%i' % (
                    ixObsID, dmy, hms, ixCCD)
...
def generate_HK(explog, vals, datapath='', elvis=context.elvis):
    ...
    for ixobs, obsid in enumerate(explog['ObsID']):
        ...
        if not skip:
            ...
            HKfile.write(HKfilef, format='ascii',
