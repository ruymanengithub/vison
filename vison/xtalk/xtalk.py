...
def plot_quad(ax, Xtalk, Q):
    ...
    ax.set_xlim((0, 6.E4))
    if Q in ['G', 'H']:
        ax.set_xlabel('source [ADU]')
    if Q in ['E', 'H']:
        ax.set_ylabel('victim [ADU]')
...
def processXtalk_single(CCDref, Qref, OBSID, thresholdinj=0., colstart=1, colend=1600,
    ...
    tCCDfileref = os.path.join(
        datapath, 'EUC_%i_*_ROE1_CCD%i.fits' % (OBSID, CCDref,))
    ...
    try:
        levels, nperlevel = find_levels(
            imgref, colstart, colend, rowstart, rowend)
    except RuntimeError:
...
class ReportXL_Xtalk(ReportXL):
    ...
    def fill_Summary(self):
        ...
        assign_item_value('# channels with max(x-talk) <= 0.5 ADU',
                          '%i/%i' % (n05, ncouples,), 1)
        assign_item_value(
            '# channels with 0.5 < max(x-talk) <= 1.0 ADU', '%i/%i' % (n05_1, ncouples,), 2)
        assign_item_value(
            '# channels with 1.0 < max(x-talk) <= 2.0 ADU', '%i/%i' % (n1_2, ncouples,), 3)
        assign_item_value(
            '# channels with 2.0 < max(x-talk) <= 5.0 ADU', '%i/%i' % (n2_5, ncouples,), 4)
        assign_item_value(
            '# channels with 5.0 < max(x-talk) <= 100.0 ADU', '%i/%i' % (n5_100, ncouples,), 5)
                ...
                for jC, CCD in enumerate(self.CCDs):
                    for jQ, Q, in enumerate(self.Quads):
                        ...
                        for k in range(3):
                            ...
                            ws['%s%i' % (jcolLetter, irow)
                               ].number_format = '0.0E+00'

                            if values[0] == 1.:
                                ws['%s%i' % (jcolLetter, irow)
                                   ].fill = self.color_fills['gray']
