...
def find_and_erase(path,keyword):
    ...
    os.system(execline1)
    print('\n')
    ans1 = raw_input('Are you happy with the selection? yes=y/Y ')
    if ans1.lower() != 'y':
        sys.exit()
    ...
    for selpath in selpaths:
        ...
        selsize = sum([sum(map(lambda fname: os.path.getsize(os.path.join(directory,fname)), files)) for directory, folders, files in os.walk(selpath)])
