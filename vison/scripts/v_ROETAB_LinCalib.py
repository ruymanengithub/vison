...
def find_discrete_voltages_inwaveform(rV,levels,filtered=None):
    ...
    for i in range(len(ulabels)):
        discrete_levels[i] = np.mean(stats.sigmaclip(rV[np.where(labels == ulabels[i])],3,3).clipped)
...
class ReportXL_RTLIN(ReportXL):
    ...
    def fill_Figures(self):
        ...
        for iCHAN,CHAN in enumerate(CHANNELS):
            ...
            self.add_image(figsdict['NL_RT'][CHAN],'Figures','M%i' % rowcounter)
...
def run_ROETAB_LinCalib(inputsfile,incatfile,datapath='', respath='', doBayes=True):
    ...
    outexcelfile = os.path.join(respath,'ROETAB_NLCAL_%s_%s.xlsx' % (Injector,Date))
    ...
    for ix,CHAN in enumerate(CHANNELS):
        ...
        if doBayes:
            ...
            bayespick = os.path.join(respath,'%s_WFfit_%s_%s_bayes.pick' % (CHAN,Injector,Date))
        ...
        figs['WF'][CHAN] = os.path.join(respath,'%s_%s_%s_WAVEFORM.png' % (CHAN,Injector,Date))
        ...
        figs['NL_RT'][CHAN] = os.path.join(respath,'%s_%s_%s_NL.png' % (CHAN,Injector,Date))
...
if __name__ == '__main__':
    ...
    parser.add_option("-c","--incat",dest="incat",default='',help="Inputs Catalog-like file.")
    parser.add_option("-i","--inputs",dest="inputs",default='',help="Master inputs file.")
    parser.add_option("-B","--Bayes",dest="doBayes",action="store_true",default=False,help="Do Bayesian Analysis?")
