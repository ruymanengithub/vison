...
class MOT_WARM(DarkTask):
    ...
    def build_scriptdict(self, diffvalues=dict(), elvis=context.elvis):
        ...
        MW_sdict['col%03i' % (ObsIDdict['BIAS_RVS'] + 1,
                              )] = dict(frames=1,
                                        exptime=0,
                                        rdmode='rwd_bas_vs',
        ...
        MW_sdict['col%03i' % (ObsIDdict['BIAS_RV'] + 1,
                              )] = dict(frames=1,
                                        exptime=0,
                                        rdmode='rwd_bas_v',
        ...
        MW_sdict['col%03i' % (ObsIDdict['RAMP'] + 1,
                              )] = dict(frames=1,
                                        exptime=0,
                                        rdmode='fwd_bas',
        ...
        for i, wavenm in enumerate(wavesPNT):
            ...
            MW_sdict['col%03i' % (ObsIDdict['PNT_%inm' % wavenm] + 1,
                                  )] = dict(frames=1,
                                            wave=FWIDx,
                                            exptime=iexptimeps,
                                            vstart=0,
    ...
    def check_metrics_ST(self, **kwargs):
        ...
        for ireg, reg in enumerate(regs_grad):
            ...
            arr = self.dd.mx['offset_%s' % reg][[BIAS_ix_RV], ...] - \
                self.dd.mx['offset_pre'][[BIAS_ix_RV], ...]
        ...
        if not self.drill:
            ...
            for jCCD, CCDk in enumerate(CCDs):
                ...
                try:
                    ...
                    for kQ, Q in enumerate(Quads):
                        profs1D2plot['FLAT'][CCDk][Q]['x'] = np.linspace(
                            1, 2086, 2086, dtype='int32')
