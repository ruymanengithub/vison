...
def gt_meta_MASK_dict(test, masktype):
    ...
    return dict(
        ...
        (ntest, masktype), meta=dict(
            doLegend=False, doColorbar=False, suptitle='%s/%s:Quadrant Images.' %
            (ntest, masktype), corekwargs=dict(
                cmap=cm.gray, aspect='auto', norm=None, origin='lower left')))
...
def get_COS_figs():
    ...
    COS_figs['Masks_MERGE'] = [
        figclasses.Fig_BeamImgShow,
        gt_meta_MASK_dict(
            'COSMETICS00',
            'MERGE')]
