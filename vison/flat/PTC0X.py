...
class PTC0X(FlatTask):
    ...
    def debugtask(self):
        ...
        if debugBloomMaps:
            ...
            foopd = pd.DataFrame.from_records(bmcdp.hdulist[2].data)

            minBM = np.nanmin(foopd.iloc[:,2:][foopd.iloc[:,2:]>0])
            maxBM = np.nanmax(foopd.iloc[:,2:][foopd.iloc[:,2:]>0])
    ...
    def debug_PRE_HER(self):
        ...
        ax1.plot(HERpreprof['E']['y'],'b-',label='pre-scan')
        ax1.plot(HERoveprof['E']['y'],'r--',label='over-scan')
        ax1.set_title('E')
        ax1.set_ylim([-2.E-2,2.E-2])
        ax1.set_xlim([10,None])
        ax2 = fig.add_subplot(222)
        ax2.plot(HERpreprof['F']['y'],'b-',label='pre-scan')
        ax2.plot(HERoveprof['F']['y'],'r--',label='over-scan')
        ax2.set_title('F')
        ax2.set_ylim([-2.E-2,2.E-2])
        ax2.set_xlim([10,None])
        ax3 = fig.add_subplot(223)
        ax3.plot(HERpreprof['G']['y'],'b-',label='pre-scan')
        ax3.plot(HERoveprof['G']['y'],'r--',label='over-scan')
        ax3.set_title('G')
        ax3.set_ylim([-2.E-2,2.E-2])
        ax3.set_xlim([10,None])
        ax4 = fig.add_subplot(224)
        ax4.plot(HERpreprof['H']['y'],'b-',label='pre-scan')
        ax4.plot(HERoveprof['H']['y'],'r--',label='over-scan')
        ax4.set_title('H')
        ax4.set_ylim([-2.E-2,2.E-2])
        ax4.set_xlim([10,None])
