...
class BF01(PTC0X):
    ...
    def extract_COV(self):
        ...
        if not self.drill:
            ...
            for jCCD,CCD in enumerate(CCDs):
                ...
                for Q in Quads: self.dd.products['COV'][CCD][Q] = OrderedDict()
                
                for ulabel in ulabels:
                
                    six = np.where(label == ulabel)                
                
                    ccdobjList = vfullinpath_adder(self.dd.mx['ccdobj_name'][six[0],jCCD])
                    
                    icovdict = covlib.get_cov_maps(ccdobjList,Npix=Npix,doTest=False)
                    
                    for Q in Quads:
                        self.dd.products['COV'][CCD][Q][ulabel] = copy.deepcopy(icovdict[Q])
    ...
    def extract_BF(self):
        ...
        if self.report is not None: self.report.add_Section(keyword='extractBF',Title='BF-Matrices Extraction',level=0)    
