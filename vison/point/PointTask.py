...
class PointTask(Task):
    ...
    def check_metrics_ST(self, **kwargs):
        ...
        if self.report is not None:
            self.addComplianceMatrix2Report(
                _compliance_flux,
                label='COMPLIANCE SATURATION TIME (PEAK FLUX)',
    ...
    def lock_on_stars(self, iObs=0, labels=None, sexconfig=None):
        ...
        if doMulti:
            LOCK_TB['LABEL'] = np.zeros(NROWS, dtype='int32')
        ...
        if not doMulti:
            ...
            for jCCD, CCDk in enumerate(CCDs):
                ...
                transf, s_list, t_list = polib._get_transformation(
                    self, ffits, stracker, dpath, _sexconfig, tag=CCDk, devel=devel)
            ...
            for jCCD, CCDk in enumerate(CCDs):
                ...
                for ilabel, label in enumerate(labels):
                    ...
                    transf, s_list, t_list = polib._get_transformation(
                        self, ffits, stracker, dpath, _sexconfig, tag='%s_%s' %
                        (CCDk, label), devel=devel)
                ...
                for jCCD, CCDk in enumerate(CCDs):
                    ...
                    for ilabel, label in enumerate(labels):
                        ...
                        if isok:
                            ...
                            ixtrue = np.where(ok_mx[jCCD, :])[0]
                            oklabel = labels[ixtrue[np.argmin(np.abs(ixtrue - ilabel))]]
                            tr = copy.deepcopy(transfs_dict[CCDk][oklabel])

                        simmx = newstrackers[CCDk][label].get_similaritymx(
                            tr.scale, tr.rotation, tr.translation)
