...
def log_prior(theta,peakrange,priors):
    ...
    areinrange = [(priors[i][0] < theta[i] < priors[i][1]) for i in range(1,len(theta))]
...
def generate_singauss(theta,x,y,spshape, Full=False):
    ...
    model = f.evaluate(x, y, peak, center_x, center_y, sigmax, sigmay, 0.).reshape(spshape)
...
def generate_airyngauss(theta,x,y,spshape, Full=False):
    ...
    adata = airy.evaluate(x, y, amplitude, center_x, center_y, radius).reshape(spshape)
    

    #2)Apply Focus
    f = models.Gaussian2D(1., center_x, center_y, focus, focus, 0.)
    
    focusdata = f.evaluate(x, y, 1., center_x, center_y, focus, focus, 0.).reshape(spshape)
    if focusdata.max() >0.:
...
def generate_ccdgauss(theta,x,y,spshape, Full=False):
    ...
    CCD = models.Gaussian2D(1., spshape[0]/2.-0.5, spshape[1]/2.-0.5, sigmax, sigmay, 0.)
    CCDdata = CCD.evaluate(x, y, 1., spshape[0]/2.-0.5, spshape[1]/2.-0.5, sigmax, sigmay, 0.).reshape(spshape)
...
def generate_ccdexp(theta,x,y,spshape, Full=False):
    ...
    CCDdata = np.exp(-(np.abs((x-center_x))/sigmax + np.abs(y-center_y)/sigmay)).reshape(spshape)
...
def generate_ccdtriangle(theta,x,y,spshape,Full=False):
    ...
    CCDdata = np.array([[0.0, weighty, 0.0],
                   [weightx, (1.-weighty-weighty-weightx-weightx), weightx],
                   [0.0, weighty, 0.0]])
...
def forwardModel(log, img, stampkey='Unknown', modeltype='gauss', gain=3.1, size=10, burn=500, spotx=2888, spoty=3514, run=700, cores=8):
    ...
    data = oimg[spoty-(size*3):spoty+(size*3)+1, spotx-(size*3):spotx+(size*3)+1].copy()
    ...
    p0[:, 0] = np.random.normal(maxs, maxs/100., size=nwalkers)                 # peak value
    ...
    if not self.drill:
        ...
        if cores >1:
            ...
            sampler = emcee.EnsembleSampler(nwalkers, ndim, log_posterior,
                                        args=[xx, yy, fdata, rn**2., peakrange, spot.shape, modeltype],
                                        pool=pool)
        else:
            sampler = emcee.EnsembleSampler(nwalkers, ndim, log_posterior,
                                        args=[xx, yy, fdata, rn**2., peakrange, spot.shape, modeltype])
    ...
    gof = (1./(np.size(data) - ndim)) * np.sum((model.flatten() - fdata)**2 / var.flatten())
